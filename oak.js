...
let memory = {
    ...
    index: {
        ...
        body: (params) => {
            ...
            let returnVal = thiss.getVariable(arrVariable.name).body[
                indexToGet
            ];
            return returnVal;
        },
    ...
    switch (returnType) {
        ...
        case "array":
            ...
            if (char === ":") {
                while (
                    this.peek() !== char ||
                    wrapperCounter !== conditionalKeywordCounter * 2 ||
                    this.isConditionalKeyword()
                ) {
                    if (this.isWrapper()) wrapperCounter++;
                    if (this.isConditionalKeyword() || this.isLoopKeyword())
                        conditionalKeywordCounter++;
                    returnValue.push(this.get());
                    testCounter++;
                }
            } else {
    ...
    switch (returnType) {
        ...
        case "array":
            ...
            if (char === ":") {
                ...
                while (
                    this.peek() !== char ||
                    openingParenCounter !== closingParenCounter
                ) {
...
Interpreter.prototype.replace = function (
    thingToReplace,
    thingToReplaceWith,
    arr
) {
...
Interpreter.prototype.functionCall = function (currentFunction) {
    if (!this.isOpeningParen())
        throw new Error(
            "A function call must have arguments wrapped in parentheses!"
        );
    this.get();
    let currentArguments = this.consumeUntil(")", "array");
    if (currentArguments.length)
        currentArguments = currentArguments.join("").split(",");
    ...
    if (currentFunction.isNative) {
        ...
        for (let i in currentArguments) {
            let funcName = currentArguments[i].substring(
                0,
                currentArguments[i].indexOf("(")
            );
            if (this.getVariable(currentArguments[i])) {
                currentFunction.params.push(
                    this.getVariable(currentArguments[i])
                );
            } else if (this.getVariable(funcName)) {
                let newInterpreter = new Interpreter(this.memory);
                currentFunction.params.push(
                    newInterpreter.input(currentArguments[i])
                );
            } else {
    ...
    for (let j = 0; j < currentArguments.length; j++) {
        let currentArgument;
        if (
            this.getVariable(currentArguments[j]) &&
            this.getVariable(currentArguments[j]).type === "fn"
        ) {
            currentArgument = currentArguments[j];
        } else {
            currentArgument =
                otherInterpreter.input(currentArguments[j]) ||
                currentArguments[j];
        }
        ...
        switch (currentFunction.params[j].type) {
            case "num":
                if (
                    typeof parseFloat(currentArgument) !== "number" ||
                    isNaN(parseFloat(currentArgument))
                )
                    throw new Error(
                        "Functions should only be called with parameters of the correct type!"
                    );
                break;
            case "str":
                ...
                break;
            case "bool":
                if (currentArgument !== "true" && currentArgument !== "false")
                    throw new Error(
                        "Functions should only be called with parameters of the correct type!"
                    );
                break;
            case "fn":
                if (
                    !this.getVariable(currentArgument) ||
                    this.getVariable(currentArgument).type !== "fn"
                )
                    throw new Error(
                        "Functions should only be called with parameters of the correct type!"
                    );
...
Interpreter.prototype.factor = function () {
    ...
    if (factorResult !== null) {
        ...
        if (!this.isClosingParen())
            throw new Error("Parentheses should always be properly closed!");
        this.get();
        return factorResult;
    ...
    if (factorResult) {
        ...
        if (variable) {
            ...
            throw new Error(
                `The identifier ${factorResult} was never declared as a variable!`
            );
        }
    }
};
...
Interpreter.prototype.comparison = function () {
    ...
    if (typeof firstExpression !== "boolean")
        throw new Error("A condition must be a boolean!");
...
Interpreter.prototype.functionDeclaration = function () {
    if (!this.isOpeningParen())
        throw new Error(
            "A function's parameters should always be wrapped in parentheses!"
        );
    this.get();
    ...
    for (let i = 0; i < functionParameters.length; i++) {
        ...
        if (
            i % 3 === 0 &&
            !["fn", "num", "str", "arr", "bool"].includes(currentElement)
        ) {
            ...
            throw new Error(
                "All function parameters must be separated by commas!"
            );
        }
    }
    functionParameters = functionParameters.filter(
        (element) => element !== ","
    );
    ...
    this.get();
    if (!this.isWrapper())
        throw new Error("A function declaration requires an opening wrapper!");
    this.get();
    let functionBody = this.consumeUntilFunctionWrapper(":", "array");
    if (!this.isWrapper())
        throw new Error("A function declaration requires a closing wrapper!");
    this.get();
...
Interpreter.prototype.variableDeclaration = function () {
    ...
    if (!this.isAssignmentOperator())
        throw new Error(
            "A variable declaration requires a valid assignment operator!"
        );
    this.get();
    switch (variableType) {
        case "fn":
        ...
        case "num":
            ...
            if (typeof variableBody !== "number")
                throw new Error("The 'num' type requires a valid number!");
            break;
        case "str":
            variableBody = this.expression();
            if (typeof variableBody !== "string")
                throw new Error("The 'str' type requires a valid string!");
            break;
        case "arr":
            variableBody = this.expression();
            variableBody = this.convertArr(variableBody);
            if (!Array.isArray(variableBody))
                throw new Error("The 'arr' type requires a valid array!");
            break;
        case "bool":
            variableBody = this.expression();
            if (typeof variableBody !== "boolean")
                throw new Error("The 'bool type requires a valid boolean!");
            break;
        default:
            throw new Error(
                "Variable assignment requires a valid variable type!"
            );
    }
...
Interpreter.prototype.conditional = function () {
    if (this.getConditionalKeyword() !== "if") {
        ...
        while (!condition) {
            if (!this.isWrapper())
                throw new Error(
                    "A conditional statement requires an opening wrapper!"
                );
            this.get();
            this.consumeUntil(":", "array");
            if (!this.isWrapper())
                throw new Error(
                    "A conditional statement requires a closing wrapper!"
                );
            this.get();
...
Interpreter.prototype.loop = function () {
    ...
    if (this.peek() === "from") {
        this.get();
        if (isNaN(this.peek()))
            throw new Error("A from loop should be followed by a number!");
        firstIndex = this.get();
        if (this.peek() !== "to")
            throw new Error(
                'A from loop should always include the "to" keyword!'
            );
        this.get();
        if (isNaN(parseInt(this.peek())))
            throw new Error(
                'The "to" keyword should always be followed by a number!'
            );
        finalIndex = this.get();
        if (this.peek() !== "with")
            throw new Error(
                'A from loop should always include the "with" keyword!'
            );
        this.get();
        if (!this.isLetter())
            throw new Error(
                'The "with" keyword should always be followed by a proper identifier!'
            );
        variableName = this.get();
        if (!this.isWrapper())
            throw new Error("A from loop should always have a proper wrapper!");
        this.get();
